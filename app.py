...
st.set_page_config(page_title="EcoPlein", page_icon="⛽", layout="wide",
                   menu_items={"About": "EcoPlein v3.1"})
...
[data-testid="stMainBlockContainer"] .stCaption{color:var(--c-text)!important;}
...
[data-testid="stExpander"] summary p,[data-testid="stExpander"] summary{color:var(--c-text)!important;}
[data-testid="stExpanderDetails"]{background:var(--c-card)!important;}
/* Alerte / info */
[data-testid="stAlert"]{background:var(--c-card)!important;border-color:var(--c-border)!important;color:var(--c-text)!important;}
/* Carte station */
.eco-card{background:var(--c-card);border:1px solid var(--c-border);border-radius:8px;
  padding:14px 16px;margin:6px 0;box-shadow:var(--shadow);transition:box-shadow .15s,background .2s;}
...
/* Prix */
.prix-cheap{display:inline-block;background:var(--c-cheap-bg);color:var(--c-cheap-fg)!important;
  border-radius:20px;padding:4px 14px;font-weight:700;font-size:1.1rem;}
.prix-avg  {display:inline-block;background:var(--c-avg-bg);  color:var(--c-avg-fg)!important;
  border-radius:20px;padding:4px 14px;font-weight:700;font-size:1.1rem;}
.prix-exp  {display:inline-block;background:var(--c-exp-bg);  color:var(--c-exp-fg)!important;
  border-radius:20px;padding:4px 14px;font-weight:700;font-size:1.1rem;}
...
/* Marque */
.brand-tag{display:inline-block;background:var(--c-brand-bg);color:var(--c-brand-fg)!important;
...
/* Services */
.svc-g{display:inline-block;background:var(--c-svc-g-bg);color:var(--c-svc-g-fg)!important;
  border-radius:12px;padding:2px 8px;font-size:.72rem;margin:2px 2px 0 0;}
.svc-b{display:inline-block;background:var(--c-svc-b-bg);color:var(--c-svc-b-fg)!important;
  border-radius:12px;padding:2px 8px;font-size:.72rem;margin:2px 2px 0 0;}
.rup-t{display:inline-block;background:var(--c-rup-t-bg);color:var(--c-rup-t-fg)!important;
  border-radius:4px;padding:2px 8px;font-size:.72rem;margin:2px 2px 0 0;}
.rup-d{display:inline-block;background:var(--c-rup-d-bg);color:var(--c-rup-d-fg)!important;
  border-radius:4px;padding:2px 8px;font-size:.72rem;margin:2px 2px 0 0;}
/* KPI */
.kpi{background:var(--c-kpi-bg);border:1px solid var(--c-border);border-radius:8px;
...
/* GPS */
.gps-ok  {background:var(--c-gps-ok-bg);color:var(--c-gps-ok-fg)!important;
  border-radius:6px;padding:8px 12px;font-size:.85rem;margin-top:8px;}
.gps-wait{background:var(--c-gps-w-bg); color:var(--c-gps-w-fg)!important;
  border-radius:6px;padding:8px 12px;font-size:.85rem;margin-top:8px;}
...
/* Légende carte */
.map-legend{display:flex;gap:14px;flex-wrap:wrap;font-size:.78rem;
  color:var(--c-muted)!important;margin-top:6px;}
/* ── Responsive ── */
...
# ── Tuiles CartoDB (pas de token Mapbox requis) ────────────────────────────────
...
def main():
    ...
    with st.sidebar:
        ...
        tc=st.radio("th",opts,index=opts.index(st.session_state.theme),
        ...
        sort_by=st.selectbox("s",
            ...
            label_visibility="collapsed")
        st.markdown("---")
        st.markdown('<small style="color:#6e7681">v3.1 · data.gouv.fr</small>',
                    unsafe_allow_html=True)
    ...
    with map_col:
        # CartoDB → aucun token Mapbox requis
        map_style=(CARTO_DARK if st.session_state.theme=="Sombre 🌙"
                   else CARTO_LIGHT
                   if st.session_state.theme=="Clair ☀️"
                   else CARTO_DARK)  # Auto → dark par défaut (suit le système)
        ...
        # S'assurer que les colonnes adresse/ville existent
        for col_name in ["adresse","ville"]:
            if col_name not in dm.columns: dm[col_name]=""
    ...
    with list_col:
        ...
        if n>20: st.info("20/20 affichées · Réduisez le rayon pour voir moins de stations")
